...
# 获取页面数据
def get_content(url, log_savepath, header = headers, proxies = False):
    html = open_url(url, header, "func: get_content", proxies)
    log_savepath = os.path.join(log_savepath, "def_get_content")
    if not os.path.exists(log_savepath):
        os.makedirs(log_savepath)
    if not html_get(url, html, log_savepath, "def get_content"):
        return None
    try:
        content = html.content
    except:
        print(url, "#get content error#")
        with open(os.path.join(log_savepath, "content_get_problem.txt"), "a+", encoding ="utf-8") as fw:
            fw.write(url + "\n")
        return None
...
# 判断链接是否正确打开
def html_get(url, html, log_savepath, where):
    if not html:
        if url:
            print("#({}) html data get errer#".format(where), url)
            with open(os.path.join(log_savepath, "html_get_problem.txt"), "a+", encoding ="utf-8") as fw:
                fw.write(url + "\n")
        else:
            print("# ({}) html data get errer, url is None #".format(where))
        return False
    return True


# 获取json文件
def get_json(url, log_savepath):
    html = open_url(url, headers, "def get_json")
    if not html_get(url, html, os.path.join(log_savepath, "html_get_problem.txt"), "def get_json"):
        return False
    html = html.text
    json_get_flag = False
    try:
        json_data = json.loads(html.encode('utf-8').decode('unicode_escape'))
        json_get_flag = True
    except:
        pass
    ...
    if not json_get_flag:
        try:
            ...
            write_log(os.path.join(log_savepath, "json_get_problem.txt"), url, "a+")
            return False
